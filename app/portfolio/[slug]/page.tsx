    ...
    <div className="cs-page">
      ...
      <header className="cs-hero">
        <div className="cs-hero-inner">
          ...
          <div style={{ display:"flex", gap:12, marginTop:24, flexWrap:"wrap" }}>
            <a href={p.url} target="_blank" rel="noopener noreferrer" className="btn btn-dark">View live site ↗</a>
            <Link href="/portfolio" className="btn btn-ghost">← All projects</Link>
          </div>
        </div>
        <div className="cs-hero-mock">
          <ProjectMockup type={p.mockupType} bg={p.mockupBg} accent={p.mockupAccent} name={p.name} size="hero" />
        </div>
      </header>
      ...
      <div className="cs-body">
        <section className="cs-section">
          ...
          <div className="cs-section-content"><p className="cs-para">{p.challenge}</p></div>
        </section>
            ...
            <ul className="cs-list">
              {p.solution.map((s,i) => (
                <li key={i} className="cs-list-item">
                  <span className="cs-list-dot" style={{ background:accent }} />{s}
                </li>
              ))}
            </ul>
          </div>
            ...
            <div className="cs-related-grid">
              {related.map(r => (
                <Link href={`/portfolio/${r.slug}`} className="cs-rel-card" key={r.slug}>
                  <div className="cs-rel-mock"><ProjectMockup type={r.mockupType} bg={r.mockupBg} accent={r.mockupAccent} name={r.name} /></div>
